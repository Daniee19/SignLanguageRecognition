...
# Modelo holístico (permite establecer los puntos)
mp_holistic = mp.solutions.holistic
mp_drawing = mp.solutions.drawing_utils  # Dibujar lineas
# Modelo para reconocer los puntos de la cara
...
def dibujar_landmarks(image, resultados):
    ...
    mp_drawing.draw_landmarks(image, resultados.face_landmarks, mp_face_mesh.FACEMESH_TESSELATION,
                              landmark_drawing_spec=mp_drawing.DrawingSpec(
                                  color=(80, 110, 10), thickness=1, circle_radius=1),  # puntos
                              connection_drawing_spec=mp_drawing.DrawingSpec(
                                  color=(80, 110, 10), thickness=1)  # líneas
...
for accion in acciones:
    ...
    for sequence in range(secuencia_video):
        ...
        for frame_num in range(secuencia_frame):
            res = np.load(os.path.join(DATA_PATH, accion, str(
        ...
        # Si es hola retorna un 0 #Se repite 30 veces por cada acción (secuencia), 30 veces 0, 30 veces 1, 30 veces 2
        labels.append(label_map[accion])

# #Datos de entrada
# .shape -> (90, 30, 1662) #toda la información en un array ejm: [accion=0[sec=0 [frag=0.npy..29.npy]]]
x = np.array(sequences)
# #Etiquetas
# se hace esto para trabajar las etiquetas con modelos de deep learning #0-> [1 0 0] #1-> [0 1 0] #2-> [0 0 1]
...
print("======================")
print("")
print("El predict con axis=1 es:", np.argmax(
    model.predict(x_test), axis=1).tolist())
print("")
...
def prob_viz(res, acciones, input_frame, colores):
    ...
    for num, prob in enumerate(res):
        cv2.rectangle(output_frame, (0, 60+num*40),
                      (int(prob*100), 90+num*40), colores[num], -1)
        cv2.putText(output_frame, acciones[num], (0, 85+num*40),
...
with mp_holistic.Holistic(min_detection_confidence=0.5, min_tracking_confidence=0.5) as holistic:
    ...
    while cap.isOpened():
        ...
        if len(sequence) == 30:
            ...
            # Dará el resultado de las probabilidades de las tres clases a lo largo de 30 fragmentos de 1660 puntos
            res = model.predict(np.expand_dims(sequence, axis=0))[0]
            # el argmax da el indice que mas probabilidad tenga similitud con la clase de los ULTIMOS 30 fragmentos hayados
            ...
            # Probabilidad visual
            imagen = prob_viz(res, acciones, imagen,
                              colores_probabilidad_dinamico)
